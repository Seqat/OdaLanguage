...
class CCodeGenerator:
    def __init__(self):
        ...
        self._var_array_depths: dict[str, int] = {}
        ...
        self._c_headers: set[str] = {"stdio.h", "stdlib.h"}

    # ── public API ───────────────────────────────────────────
    def generate(self, program: ast.Program) -> str:
        self._c_headers.update(getattr(program, "c_headers", set()))
        # Discover classes and functions first
        for s in program.statements:
            if isinstance(s, ast.ClassDeclaration):
                self._class_names.add(s.name)
                self._class_field_types[s.name] = {
                    field.name: field.type_ann.base_type
    ...
    def _assemble(self) -> str:
        for var_name in self._heap_vars:
            dims = self._heap_var_dims.get(var_name)
            if dims and len(dims) > 1:
                self._emit_free_helper(len(dims))

        include_order = ["errno.h", "math.h", "stdio.h", "stdlib.h", "string.h"]
        lines = ["/* Generated by OdaLanguage Transpiler v0.1 */"]
        for header in include_order:
            if header in self._c_headers:
        ...
        while self._heap_vars:
            ...
            lines.append(f"    {self._heap_cleanup_stmt(var_name)}")
            self._forget_heap_var(var_name)
    ...
    def _emit_conversion_helpers(self):
        if "_oda_to_str_helpers" in self._helpers_emitted:
            return
        self._helpers_emitted.add("_oda_to_str_helpers")
        self._top += [
            "static ODA_UNUSED char* _oda_int_to_str(int v) {",
            "    char* buf = (char*)malloc(24);",
            "    snprintf(buf, 24, \"%d\", v);",
    ...
    def _emit_str_concat_helper(self):
        if "_oda_str_concat" in self._helpers_emitted:
            return
        self._helpers_emitted.add("_oda_str_concat")
    ...
    def _emit_input_helper(self):
        if "_oda_input" in self._helpers_emitted:
            return
        self._helpers_emitted.add("_oda_input")
        self._top += [
            "static ODA_UNUSED char* _oda_input() {",
            "    size_t cap = 64;",
            "    size_t len = 0;",
    ...
    def _emit_read_file_helper(self):
        if "_oda_read_file" in self._helpers_emitted:
            return
        self._helpers_emitted.add("_oda_read_file")
        self._c_headers.add("errno.h")
        self._top += [
            "static ODA_UNUSED char* _oda_read_file(const char* path) {",
            "    errno = 0;",
            "    FILE* f = fopen(path, \"r\");",
    ...
    def _emit_assert_helper(self):
        if "_oda_assert" in self._helpers_emitted:
            return
        self._helpers_emitted.add("_oda_assert")
        self._top += [
            "static ODA_UNUSED void _oda_assert(int condition) {",
            "    if (!condition) {",
            "        fprintf(stderr, \"oda: assertion failed\\n\");",
            "        exit(1);",
            "    }",
            "}",
            "",
    ...
    def _emit_free_helper(self, depth: int):
        """Recursively generate _oda_free_Nd helpers if needed."""
        if depth < 2:
            return
        func_name = f"_oda_free_{depth}d"
        if func_name in self._helpers_emitted:
            return

        if depth > 2:
            self._emit_free_helper(depth - 1)
        ...
        stars = "*" * depth
        lines = [
            f"static void {func_name}(void{stars} arr, {args}) {{",
            "    if (!arr) {",
            "        return;",
            "    }",
        ]
        ...
        if depth == 2:
            ...
            lines += [
                "    for (int i = 0; i < d1; i++) {",
                f"        _oda_free_{depth-1}d((void{inner_stars})arr[i], {inner_args});",
                "    }",
            ]
    ...
    def _emit_func_def(self, func: ast.FuncDeclaration):
        ...
        if func.is_extern:
            if func.extern_header or func.name in {"print", "input", "assert"}:
                return
            self._top.append(f"{ret} {func.name}({params});")
            return
    ...
    def _emit_block(self, stmts: list, out: list[str], class_ctx: str | None = None):
        ...
        if exits_with_return:
            while len(self._heap_vars) > heap_start:
                self._forget_heap_var(self._heap_vars.pop())
        else:
    ...
    def _heap_cleanup_stmt(self, var_name: str) -> str:
        ...
        if dims and len(dims) > 1:
            depth = len(dims)
            self._emit_free_helper(depth)
            cast = f"(void{'*' * depth})"
            return f"_oda_free_{depth}d({cast}{var_name}, {', '.join(dims)});"
        return f"free({var_name});"

    def _forget_heap_var(self, var_name: str):
        self._heap_var_dims.pop(var_name, None)
    ...
    def _array_type_for_expr(self, node) -> ast.TypeAnnotation | None:
        if isinstance(node, ast.Identifier):
            depth = self._var_array_depths.get(node.name)
            if depth:
                return ast.TypeAnnotation(
                    base_type=self._var_types.get(node.name, "int"),
                    is_array=True,
                    array_depth=depth,
                )
        if isinstance(node, ast.IndexAccess):
            outer = self._array_type_for_expr(node.obj)
            if outer and outer.array_depth > 1:
                return ast.TypeAnnotation(
                    base_type=outer.base_type,
                    is_array=True,
                    array_depth=outer.array_depth - 1,
                )
        return None
    ...
    def _emit_var_decl(self, v: ast.VarDeclaration, out: list[str], class_ctx=None):
        ...
        if v.type_ann.is_array:
            ...
            if v.initializer:
                ...
                if not dims and isinstance(v.initializer, ast.ArrayAllocation):
                    dims = [self._expr(size, class_ctx) for size in v.initializer.sizes]
            ...
            if v.initializer:
                ...
                if self._expr_allocates_heap(v.initializer):
                    if v.type_ann.array_depth > 1 and dims:
                        self._heap_var_dims[v.name] = dims
                        self._emit_free_helper(v.type_ann.array_depth)
        ...
        if isinstance(arg, ast.StringLiteral):
            ...
            for part in arg.parts:
                if isinstance(part, str):
                    ...
                    t = self._infer_expr_type(part, class_ctx)
            ...
            t = self._infer_expr_type(arg, class_ctx)
            fmt = self._get_fmt(t)
    ...
    def _expr(self, node, class_ctx=None, *, owns_result: bool = False) -> str:
        ...
        if isinstance(node, ast.AssignExpr):
            ...
            target_array_type = self._array_type_for_expr(node.target)
            if target_array_type and isinstance(node.value, ast.ArrayLiteral):
                v = self._emit_array_expr(node.value, target_array_type, class_ctx)
            else:
    ...
    def _emit_call_expr(self, call: ast.CallExpr, class_ctx=None) -> str:
        if isinstance(call.callee, ast.Identifier):
            ...
            if name == "input":
                ...
                return "_oda_input()"
            if name == "assert":
                self._emit_assert_helper()
                args_str = ", ".join(self._expr(a, class_ctx) for a in call.args)
                return f"_oda_assert({args_str})"
    ...
    def _looks_like_string(self, node, class_ctx=None) -> bool:
        if isinstance(node, (ast.StringLiteral, ast.InterpolatedString)):
            return True
        if isinstance(node, ast.Identifier):
            t = self._infer_expr_type(node, class_ctx)
            if t == "string":
                return True
        if isinstance(node, ast.MemberAccess):
            return self._infer_expr_type(node, class_ctx) == "string"
        if isinstance(node, ast.BinaryExpr) and node.op == "+":
            return self._looks_like_string(node.left, class_ctx) or self._looks_like_string(node.right, class_ctx)
        return False
