...
- **(T10, CRITICAL) returning a heap string is a use-after-free.** The return path frees all function-level heap vars including the returned lvalue → `free(s); return s;`. Returning a class with heap fields by value has the same shape (local destructor runs on an escaping copy). Intended fix: skip the free for the returned identifier (ownership transfers out; caller leak is acceptable v1, UAF is not); reject heap-field class return-by-value in semantic.py.
