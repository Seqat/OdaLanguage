...
- (Closed by T11) type-soundness trio + nullable decision (a): `infer_binary_type` strips a trailing `?` for `??` (right arm must equal/widen to the stripped type, else `None`→E3037); string `==`/`!=` lower to `strcmp(...) ==/!= 0` in `_expr` (routes operands through `_ensure_str`, adds `string.h`); value-scalar nullables are rejected at var-decl with `E3048` (string/class/array only); nullable values reaching `print`/interpolation/concat unguarded are rejected with `E3049` (`_reject_nullable_use`). Covered by `tests/test_integration.py` + `tests/test_semantic_negative.py`.
